...
为了让项目的进度变得直观，本系统设计了一套由 6 级内部状态驱动的生命周期管理机制。在数据库中，项目状态从草案（0）到已结项（4）不等。前端在接收到后端返回的状态枚举后，会利用 Vue 的计算属性动态匹配对应的彩色标签，具体展示效果如图\ref{fig:project_tags}所示。比如“执行中”显示为绿色，“待审批”显示为黄色，这样科研人员一眼就能看清自己负责的所有项目正处于什么阶段。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth,totalheight=0.35\textheight,keepaspectratio]{figures/project_tags.png}
...
\end{enumerate}
科研人员视角下的多维预警标签在系统列表页面的具体表现如图\ref{fig:intelligent_tags_researcher}所示。
...
\end{enumerate}
管理人员端侧重于任务驱动与资产审计的智能标签配置如图\ref{fig:intelligent_tags_admin}所示。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth,totalheight=0.25\textheight,keepaspectratio]{figures/intelligent_tags_researcher.png}
    \caption{科研人员视角下的智能风险标签展示}
    \label{fig:intelligent_tags_researcher}
    
    \vspace{0.8em}
    
    \includegraphics[width=0.75\textwidth,totalheight=0.25\textheight,keepaspectratio]{figures/intelligent_tags_admin.png}
...
在支出记录模块，系统通过 AOP 拦截与实时余额检索，实现了经费报销的动态平衡控制。当报销金额超过分项余额、或项目已逾期时，前端会实时反馈高亮警告，具体拦截效果如图\ref{fig:expenditure_alert}所示。

%此处需要一张“报销录入页-余额预警”的截图
%建议操作：在报销录入界面，选择一个余额不足的项目录入大额支出，截取页面顶部的红色/黄色警告框，命名为 expenditure_alert.png
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,totalheight=0.35\textheight,keepaspectratio]{figures/expenditure_alert.png}
    \caption{基于预算阈值的报销录入硬拦截机制}
    \label{fig:expenditure_alert}
\end{figure}

\subsection{智能助手风险扫描机制}
“智能助手”模块是本系统的技术亮点之一，它通过主动巡检模式识别潜在的财务风险，其风险建议面板与详情的展示如图\ref{fig:assistant}与图\ref{fig:assistant_detail}所示。其实现逻辑如下代码所示：
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth,totalheight=0.45\textheight,keepaspectratio]{figures/assistant.png}
    \caption{智能助手风险概览面板}
    \label{fig:assistant}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth,totalheight=0.45\textheight,keepaspectratio]{figures/assistant_detail.png}
    \caption{智能助手风险分析详情}
    \label{fig:assistant_detail}
\end{figure}

\subsection{全校执行风险监控看板}
针对管理层，系统实现了“全校风险监控”功能。该看板集成了多维维度的全量数据扫描能力，如图\ref{fig:admin_monitor_combined}所示，管理员可通过“视角切换器（Researcher Selector）”下钻查看特定科研人员的项目细节，实现了从“宏观总览”到“微观审计”的无缝衔接。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth,totalheight=0.8\textheight,keepaspectratio]{figures/admin_monitor_combined.png}
...
该算法保证了进度的客观性。当前端检测到执行率 $\text{FundRate}$ 显著低于 $\text{TimeProgress}$ 时，系统会自动触发“执行滞后”预警，如图\ref{fig:execution_compare}所示，为管理人员提供深度的干预参考。

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,totalheight=0.4\textheight,keepaspectratio]{figures/execution_compare.png}
...
在前端 \texttt{DashboardView.vue} 中，系统通过响应式配置项（Options）实现了数据看板的实时渲染。特别是“月度支出趋势与预测”的逻辑呈现如图\ref{fig:trend_forecast}所示。还有“经费执行率与时间进度”的对比图，采用了重叠条形图设计。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,totalheight=0.35\textheight,keepaspectratio]{figures/trend_forecast.png}
...
此外，为了揭示支出构成的合理性，系统同步集成了经费科目占比分析饼图，如图\ref{fig:category_share_pie}所示。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth,totalheight=0.35\textheight,keepaspectratio]{figures/category_share_pie.png}
    \caption{多维度经费支出占比决策分析}
    \label{fig:category_share_pie}
\end{figure}

\section{本章小结}
本章讨论了科研经费管理系统的核心代码实现与可视化集成工作。通过后端基于 Spring Boot 的服务编排和前端基于 ECharts 的数据驱动，构建了从底层数据采集、多维风险扫描到顶层决策展示的完整工程路径。特别是“拦截机制”与“智能助手预警模型”，验证了该系统在高校财务数字化转型实践中的可行性。
